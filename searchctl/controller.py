...
async def run(
    ...
    try:
        ...
        # Periodic 360 scan station — SLOW version (was 120 deg/s for 3.5s in
        # 9f5428a + 4066293 which the user reported caused wall crashes).
        # Now 60 deg/s for 6 s = exactly 360 deg, no overshoot. Slower yaw
        # = better vision-EKF tracking. Pure rotation at fixed XY, no
        # altitude wiggle, no descent.
        SCAN_EVERY_S      = 60.0
        SCAN_DURATION_S   = 6.0
        SCAN_YAW_RATE_DEG = 60.0
        ...
        # Position-stuck watchdog: if drone moved <0.3m in last 20s AND we're
        # past the initial 45s grace window, FSM has failed to make progress.
        ...
        # Tuned to be "essentially stationary for 20s" — corner-turn
        # maneuvers and FSM escape recoveries should still slip under it.
        STUCK_WATCHDOG_WINDOW_S = 20.0
        STUCK_WATCHDOG_MOVE_M   = 0.3
        STUCK_WATCHDOG_GRACE_S  = 45.0
