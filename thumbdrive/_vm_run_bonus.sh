#!/bin/bash
# Bonus-mode run: wall-follow tuned for the qualifier 5-min bonus window.
# Hard-lands at ~4:20, early-exits when both yellow + red detected,
# bumps K's wall-follow speeds.
#
# Use this for the FIRST qualifier attempt. If it doesn't bank enough
# detections, fall back to _vm_run_wall.sh (defensive 8-min long run).
set +e
SESS=au
