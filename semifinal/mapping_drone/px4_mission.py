...
flight + pose layer to ``px4_ros.Px4Ros2Flight``.

RUN PREREQUISITES (on the drone)
--------------------------------
    source ~/ros2_ws/install/setup.bash         # so px4_msgs imports
...
THREE MODES (always test in this order)
---------------------------------------
    --check    Connect, print live pose + arming/offboard status for a few
               seconds, then exit. NEVER arms. Use this FIRST to confirm the
               ROS2 link + that PX4 has a valid position estimate.
    --nofly    Real pose + RealSense + ArUco + occupancy + artifacts, looped
               in place. NEVER arms/takes off/moves. Grounded detection test
               and the full Challenge-1 pipeline if the drone is flown
               MANUALLY. (Default.)
    --fly      Autonomous offboard survey: stream setpoints -> engage offboard
               -> arm -> fly the waypoints (scanning at each) -> land -> disarm.
               Only after --check and --nofly look right, and with a human on a
               kill switch.

Examples
--------
    python3 -m mapping_drone.px4_mission --check
    python3 -m mapping_drone.px4_mission --nofly --aruco-dict 7X7_1000
    MAPPING_DRONE_VALIDITY=lookup MAPPING_DRONE_VALIDITY_LOOKUP=configs/valid_ids_10jun.json \
...
# Reuse the controller's tuning where it makes sense.
...
class Px4Mission:
    def __init__(self, args: argparse.Namespace, flight: Px4Ros2Flight,
                 realsense: RealsenseNode, run_writer: RunWriter) -> None:
        self.args = args
        self.flight = flight
    ...
    def _write_status(self) -> None:
        n, e, down, yaw, valid = self.flight.get_pose()
    ...
    def _register_sighting(self, frame: RealsenseFrame, aruco_id: int,
                           pixel_center, bbox) -> None:
        u, v = pixel_center
        ...
        if 0 <= u < w and 0 <= v < h:
            ...
            if depth_mm > 0:
                n, e, down, yaw, _ = self.flight.get_pose()
                try:
                    cam_xyz = deproject_pixel_to_camera_xyz(
                        frame.intrinsics, u, v, depth_mm / 1000.0)
                    world_xyz = camera_to_world(
                        cam_xyz, n, e, -down, yaw, self.args.gimbal_pitch)
                    confidence = 1.0
                except Exception as exc:
        ...
        try:
            img_path = self.run_writer.save_marker_image(
                frame.color_bgr, aruco_id, self._seq, bbox_xyxy=bbox)
        except Exception as exc:
    ...
    def _await_pose(self, timeout_s: float) -> bool:
        ...
        while time.monotonic() < deadline and not self._stop:
            _, _, _, _, valid = self.flight.get_pose()
            if valid and self.flight.last_pos_ts > 0:
                return True
            time.sleep(0.2)
        return False
    ...
    def run_check(self) -> int:
        logger.info("CHECK MODE: printing PX4 pose + status for ~10 s, never arms")
        deadline = time.monotonic() + 10.0
        while time.monotonic() < deadline and not self._stop:
            n, e, down, yaw, valid = self.flight.get_pose()
            logger.info("pose n=%.3f e=%.3f down=%.3f yaw=%.1f valid=%s | armed=%s offboard=%s",
                        n, e, down, yaw, valid, self.flight.armed, self.flight.in_offboard)
            time.sleep(1.0)
        if self.flight.last_pos_ts == 0.0:
            logger.error("CHECK: never received /fmu/out/vehicle_local_position — is the XRCE "
                         "agent up and px4_msgs sourced?")
            return 2
        return 0
    ...
    def run_nofly(self) -> int:
        ...
        if not self._await_pose(POSE_WAIT_S):
            logger.warning("no /fmu pose yet — continuing (world coords may be invalid)")
    ...
    def run_fly(self, waypoints) -> int:
        ...
        # 1. stream setpoints (hold current pose) before switching to offboard.
        n0, e0, down0, yaw0, _ = self.flight.get_pose()
        self.flight.set_target(n0, e0, self.args.takeoff_alt, yaw0)
        self.flight.begin_streaming()
        self.state = "OFFBOARD_PREWARM"
        time.sleep(1.0)  # ~20 setpoints at 20 Hz

        # 2. engage offboard + arm, confirm.
        self.state = "ARMING"
        ...
        # 3. climb to takeoff alt at current xy.
        self.state = "TAKEOFF"
    ...
    def _fly_to(self, n, e, alt, yaw_deg) -> bool:
        ...
        while time.monotonic() < deadline and not self._stop:
            cn, ce, cdown, _, _ = self.flight.get_pose()
    ...
    def _land_and_disarm(self, *, aborted: bool) -> int:
        ...
        # give it time to descend, then disarm as a backstop.
        for _ in range(40):
...
def _parse_args(argv=None) -> argparse.Namespace:
    ...
    mode.add_argument("--fly", action="store_true", help="autonomous offboard survey")
...
def main() -> int:
    ...
    if not px4_available():
        logger.error("px4_msgs/rclpy not importable. On the drone run: "
    ...
    logger.info("ArUco dict: %s | validity: %s", args.aruco_dict, describe_rule())
    ...
    try:
        flight.start()
        realsense.start()
        mission = Px4Mission(args, flight, realsense, run_writer)
        ...
        aborted = True
        # best-effort land if we might be flying
        if args.fly and mission is not None:
            try:
        ...
        try:
            run_writer.finalise(mission.grid if mission else OccupancyGrid(0.05, 20.0),
